...
static int parse_packet(ul_parser_t *parser, const uint8_t *packet,
                        size_t packet_len, ul_header_t *out_hdr,
                        uint8_t *out_payload) {
...
static int test_non_fragmented(uint16_t seq, const uint8_t *payload,
                               size_t payload_len) {
  uint8_t packet[2048];
  ...
  int parse_res =
      parse_packet(&parser, packet, (size_t)packet_len, &rx_hdr, rx_payload);
  if (parse_res != 1) {
    return -1000;
  }
...
static int test_fragmented(uint16_t *seq, const uint8_t *payload,
                           size_t payload_len) {
  ul_header_t base = {0};
  base.payload_len = 0;
  ...
  for (int i = 0; i < num_frags; i++) {
    ...
    int packet_len =
        uavlink_pack(packet, &frags.headers[i], frags.payloads[i], NULL);
    if (packet_len < 0) {
      return -2001;
    }
    ...
    int reasm_res =
        ul_reassembly_add(&reasm_ctx, &rx_hdr, rx_payload, rx_hdr.payload_len,
...
int main(void) {
  ...
  size_t fixed_sizes[] = {0,   1,   2,   10,   64,   128, 255,
                          256, 257, 512, 1024, 2048, 4095};
